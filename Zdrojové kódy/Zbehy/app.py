    ...
    def run(self):  # základ vlákna
        while not self.isInterruptionRequested():
            ...
            if data is not None:
                ...
                self.odhlaskaLo = data["TS"][
                    "odhlLo"
                ]  # čítanie odhlášok z hradla Rišňovce
                ...
                if data["Cesta"]["stavanie"]:
                    ...
                    ) and (data["Cesta"]["odosielatel"] != "ZBE"):
                ...
                if (
                    ...
                    and data["Navest"]["odosielatel"] != "HLO"
                ):
...
class App(QMainWindow):  # hlavná triedy vizualizácie
    ...
    def update(
        ...
        if objekt in ["update", "navestidla"]:  # aktualizácia návestidiel
            self.dictNav[28].zhasnute = not self.workerThread.dictTS[
                2
    ...
    def clickObjekt(self, id, objekt):  # metóda spracovávajúca kliknutie na objekt
        if objekt == "navestidlo":
            ...
            if not self.workerThread.dictStanice[
                2
            ].dialkove:  # ak má precovisko aktívne riadenie
            ...
            if not self.workerThread.dictStanice[
                ...
                if (
                    id in self.workerThread.dictUseky
                ):  # ak sa výhybka nachádza v zozname
    ...
    def akciaNavestidlo(
        ...
        if (ID in [1, 2]) and index == 1:  # výber vlakovej cesty
        ...
        elif ((ID == 2 and index == 2) or (ID == 3 and index == 1)) and (
            self.lastNav not in [16, 17, 25]
            ...
            self.Start in [12, 13, 27] and ID == 10 and index == 1
            ...
            (ID == 10) and (index == 2) and (not self.szz.typCesty)
        ...
        elif (self.Start in [20, 21, 22, 23]) and (
            (ID == 12) and (index == 1) and (not self.szz.typCesty)
            ...
            (self.Start in [18, 19, 20, 21, 22, 23, 24])
            and ((ID == 10) and (index == 3))
            or ((ID == 11) and (index == 1) and self.szz.typCesty)
    ...
    def akciaTS(self, index, id):  # metóda pre prácu s traťovým súhlasom
        ...
        if (index == 1) and (id in [2, 4]):  # žiadosť o TS
            if (not self.workerThread.dictTS[id].prijem) and (
                not self.workerThread.dictTS[id].odchod
            ):
            ...
            (
                (index == 3)
                and (id == 2)
                and (self.workerThread.dictTS[2].ziadost is True)
            )
            or (
                (index == 3)
                and (id == 4)
                and (self.workerThread.dictTS[4].ziadost is True)
            )
            or (
                (index == 1)
                and (id == 3)
                and (self.workerThread.dictTS[3].prijem is True)
            )
        ):  # udelenie TS
            self.prikazDoPLC("UTS/" + str(id))
    ...
    def akciaStanica(self, index):  # metóda pre spracovanie signálov riadenia
        if index == 1:  # žiadosť o prevzatie riadenia
            if self.workerThread.dictStanice[self.lastStanica].dialkove:
                ...
                self.vypisHlasenia(
                    "Obsluha stanice prevedená na pracovisko lokálnej obsluhy"
                )
...
if __name__ == "__main__":
    ...
    widget.ui.combo_TS_RAD.currentIndexChanged.connect(
        lambda: widget.akciaTS(widget.ui.combo_TS_RAD.currentIndex(), 2)
    )  # traťový súhlas
    widget.ui.combo_TS_HLO.currentIndexChanged.connect(
        lambda: widget.akciaTS(widget.ui.combo_TS_HLO.currentIndex(), 3)
    )
    widget.ui.combo_TS_LUZ.currentIndexChanged.connect(
        lambda: widget.akciaTS(widget.ui.combo_TS_LUZ.currentIndex(), 4)
    )
    ...
    widget.ui.actionZavrie.triggered.connect(
        lambda: widget.quit()
    )  # terminácia aplikácie

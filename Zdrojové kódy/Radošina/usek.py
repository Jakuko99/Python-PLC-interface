            ...
            if self.jeVolny:  # ak je úsek voľný
                if self.stavanieVlak:  # ak je úsek vybratý pre vlakovú cestu
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.STAVANIE_VC.value]
                    )

                elif self.stavaniePosun:  # ak je úsek vybratý pre posunovú cestu
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.STAVANIE_PC.value]
                    )

                elif self.stavanieOchr:  # ak je úsek vybratý pre ochrannú
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.STAVANIE_OD.value]
                    )

                elif self.vlak:  # ak je úsek vo vlakovej ceste
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.VLAK.value]
                    )
                    ...
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.OCHR.value]
                    )

                elif self.vyberVlak:  # ak prebieha výber vlakovej cesty
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.VYBER_VC.value]
                    )

                else:  # základné zobrazenie úseku
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.VOLNA.value]
                    )

            else:  # ak je úsek obsadený
                getattr(self.app.ui, self.nazovGUI).setIcon(
                    self.dictIkon[self.enumIkon.OBSAD.value]
                )

        else:  # informácia o úseku neexistuje
            getattr(self.app.ui, self.nazovGUI).setIcon(
                self.dictIkon[self.enumIkon.MRTVA.value]
            )

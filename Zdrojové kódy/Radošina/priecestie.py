    ...
    def update(self, app):
        if self.jeVolny:  # ak je úsek voľný
            if self.otvorene:  # ak je priecestie otvorené
                if self.vyber:  # ak je vybrané užívateľom
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.OTVORENE_VYBER.value]
                    )

                else:  # základný obraz otvoreného priecestia
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.OTVORENE.value]
                    )

            elif self.zatvorene:  # ak je priecestie zatvorené
                if self.vyber:  # ak je vybrané užívateľom
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.ZATVORENE_VYBER.value]
                    )

                else:  # základný obraz zatvoreného priecestia
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.ZATVORENE.value]
                    )

            elif self.predzvananie:  # ak plynie predzváňací čas
                getattr(self.app.ui, self.nazovGUI).setIcon(
                    self.dictIkon[self.enumIkon.PREDZVANANIE.value]
                )
            ...
            if self.zatvorene:  # ak je priecestie zatvorené
                if self.vyber:  # ak je vybrané užívateľom
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.ZATVORENE_VYBER_OBS.value]
                    )

                else:  # základný obraz zatvoreného priecestia
                    getattr(self.app.ui, self.nazovGUI).setIcon(
                        self.dictIkon[self.enumIkon.ZATVORENE_OBS.value]
                    )

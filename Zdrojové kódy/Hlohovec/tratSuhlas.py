            ...
            if not self.odchod and self.volnost:  # ak je medzistaničný úsek voľný
                if self.prijem:  # ak je TS prijatý
                    if self.ziadost:  # ak prebieha žiadosť o TS
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.PRIJEM_ZIADOST.value]
                        )

                    elif self.porBP:  # ak došlo k poruche blokovej podmienky
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.PRIJEM_VOLNOST_PORUCHA_BP.value]
                        )

                    else:  # základný obraz TS
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.PRIJEM_VOLNOST.value]
                        )

                else:  # ak je TS udelený
                    if self.ziadost:  # ak prebieha žiadosť o TS
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.UDELENIE_ZIADOST.value]
                        )

                    elif self.porBP:  # ak došlo k poruche blokovej podmienky
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[
                                self.enumIkon.UDELENIE_VOLNOST_PORUCHA_BP.value
                            ]
                        )

                    else:  # základný obraz TS
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.UDELENIE_VOLNOST.value]
                        )
                ...
                if self.prijem:  # ak je TS prijatý
                    if self.porBP:  # ak došlo k poruche blokovej podmienky
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.PRIJEM_PORUCHA_BP.value]
                        )

                    else:
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.PRIJEM.value]
                        )

                else:  # ak je TS udelený
                    if self.porBP:  # ak došlo k poruche blokovej podmienky
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.UDELENIE_PORUCHA_BP.value]
                        )

                    else:
                        getattr(self.app.ui, self.nazovGUI).setIcon(
                            self.dictIkon[self.enumIkon.UDELENIE.value]
                        )

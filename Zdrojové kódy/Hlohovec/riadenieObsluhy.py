        ...
        if self.dialkove:  # diaľkové ovládanie stanice
            if self.ziadost:  # ak je aktívna žiadosť o lokálnu obsluhu
                getattr(self.app.ui, self.nazovGUI).setIcon(
                    self.dictIkon[self.enumIkon.DIALKOVE_ZIADOST.value]
                )

            else:  # základný obraz
                getattr(self.app.ui, self.nazovGUI).setIcon(
                    self.dictIkon[self.enumIkon.DIALKOVE.value]
                )

        else:  # lokálne ovládanie stanice
            if self.ziadost:  # ak je aktívna žiadosť o lokálnu obsluhu
                getattr(self.app.ui, self.nazovGUI).setIcon(
                    self.dictIkon[self.enumIkon.MIESTNE_ZIADOST.value]
                )

            else:  # základný obraz
                getattr(self.app.ui, self.nazovGUI).setIcon(
                    self.dictIkon[self.enumIkon.MIESTNE.value]
                )

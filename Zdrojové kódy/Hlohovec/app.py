        ...
        while not self.isInterruptionRequested():
            ...
            if data is not None:
                ...
                for i in [26]:  # čítanie smeru výmen
                    self.dictUseky[i].smer = data["SmerVyh"][
                        self.dictUseky[i].IDsmer
                    ]  # informácia o aktuálnom smere výmeny
                ...
                self.odhlaskaSo = data["TS"][
                    "odhlSo"
                ]  # čítanie odhlášok z hradla Rišňovce

                self.predhlaskaZBE = data["TS"][
                    "odchodZH"
                ]  # čítanie predhlášok zo susedných staníc
                ...
                if data["Cesta"]["stavanie"]:
                    ...
                    ) and (data["Cesta"]["odosielatel"] != "HLO"):
...
class App(QMainWindow):  # hlavná triedy vizualizácie
    ...
    def update(
        ...
        if objekt in ["update", "priecestie"]:  # aktualizácia priecestí
            if (
                clicked and ID in self.workerThread.dictPriecestie
            ):  # ak bolo návestidlo kliknuté obsluhou
    ...
    def akciaNavestidlo(
        ...
        if ((ID in [1, 2]) and index == 1) and (
        ...
        elif (self.Start == self.dictNav[37].ID) and (
            ...
            (ID in [10, 12]) and (index == 2) and (not self.szz.typCesty)
        ):  # stavanie vchodovej cesty s ochrannou dráhou
            self.vypisHlasenia("Jazdná cesta nemá definovanú ochrannú dráhu")
            self.ukonciStavanie()

        elif (self.Start in [self.dictNav[38].ID, self.dictNav[39].ID]) and (
            (ID in [10, 12]) and (index == 1) and (not self.szz.typCesty)
    ...
    def akciaTS(self, index, id):  # metóda pre prácu s traťovým súhlasom
        if (
            (index == 1) and (id == 5) and (self.workerThread.dictTS[id].prijem is True)
        ):  # udelenie TS
...
if __name__ == "__main__":
    ...
    widget.ui.combo_TS_HLO.currentIndexChanged.connect(
        lambda: widget.akciaTS(widget.ui.combo_TS_HLO.currentIndex(), 5)
    )  # traťový súhlas

...
dataNavest = {"ID": 0, "znak": "Stoj", "odosielatel": "X"}
...
@app.put(
    "/Cesta/{start}/{end}/{typ}/{od}/{stav}/{rus}/{meno}"
)  # metóda spracovávajúca časové pečiatky z dispečerskej aplikácie a staničnej aplikácie Zbehy
async def writeCesta(
    start: int, end: int, typ: bool, od: bool, stav: bool, rus: bool, meno: str
):
...
@app.put(
    "/ziadostZBP/{value}"
)  # metóda spracovávajúca žiadosti o blokovej podmienke od ESA-44
...
async def writeESA():
    ...
    if ((aktualnyCas - casovaPeciatkaILTISdisp) <= timedelta(seconds=15)) or (
        (aktualnyCas - casovaPeciatkaILTISzbe) <= timedelta(seconds=15)
    ):  # ak je posledná správa od ILTIS-N s platným časom
        casPeciatka = True
...
@app.put(
    "/volnost/{value}"
)  # metóda pre ESA 44 na zápis obsadenosti medzstaničného úseku
...
@app.put(
    "/ziadRiad/{index}/{value}"
)  # metóda spracovávajúca žiadosti o zmenu ovládania medzi aplikáciami
...
@app.put(
    "/udelRiad/{index}"
)  # metóda spracovávajúca pokyny pre zmenu ovládania medzi aplikáciami
...
async def read():
    ...
    if not (zapis and citanie):
        ...
        if RiseEdgeR.detect(
            bool(bin_array_M7_volnost[3])
        ):  # rušenie prízaku odchodovej cesty po uovoľnení medzistaničného úseku
        ...
        dataTS["volRZ"] = FlipFlop(
            Set=bool(
                bool(bin_array_M7_volnost[3])
                and not (dataTS["odchodR"] or dataTS["odchodZR"])
            ),  # SR obvod pre správnu indikáciu voľnosti trate
            Reset=(
                dataTS["odchodR"]
                or dataTS["odchodZR"]
                or not (bool(bin_array_M7_volnost[3]))
            ),
        )

        dataTS["volZH"] = FlipFlop(
            Set=bool(bool(bin_array_M7_volnost[4]) and dataTS["odhlLo"]),
            Reset=(
                dataTS["odchodZH"]
                or dataTS["odchodH"]
                or not (bool(bin_array_M7_volnost[4]))
            ),
        )

        dataTS["volLZ"] = FlipFlop(
            Set=usekZL and (not dataTS["odchodL"]) and (not dataTS["odchodZL"]),
            Reset=(dataTS["odchodL"] or dataTS["odchodZL"] or not usekZL),
        )

        dataTS["predhlLo"] = bin_array_M29_PreOdh[
            2
        ]  # načítanie predhlášok a odhlášok z AH Rišňovce
        ...
        aktualnyCas = (
            arrow.now()
        )  # porovnanie časovej platnosti poslednej správy z ESA 44
        ...
        return {
            "RAD;RAD-ZBE": bin_array_I1_usek,
            "ZBE": bin_array_I2_usek,
            "P1;P2;ZBE-HLO": bin_array_I3_usek,
            "HLO": bin_array_I4_usek,
            "ZbeLuz": muZbeLuz,
            "SmerVyh": bin_array_M5_smerVyh,
            "Priecestie": bin_array_M1_priecestie,
            "TS": dataTS,
            "Riadenie": dataRiadenie,
            "Cesta": dataCesta,
            "Navest": dataNavest,
        }
...
@app.put(
    "/write/navestidlo/{stanica}/{meno}/{znak}"
)  # metóda pre zmenu návestného znaku na návestidlách
...
@app.put(
    "/write/OchrDr/{meno}/{value}"
)  # metóda re zápis stavaných ochranných dráh vlakových ciest
